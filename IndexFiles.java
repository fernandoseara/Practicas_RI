...
public class IndexFiles implements AutoCloseable {
	...
	void indexDoc(IndexWriter writer, Path file, long lastModified) throws IOException {
		try (InputStream stream = Files.newInputStream(file)) {
			...
			//contentsStored with same data as contents but field stored.
			doc.add(new TextField("contentsStored",
					new BufferedReader(new InputStreamReader(stream, StandardCharsets.UTF_8), Field.Store.YES)));
			...
			try{
			   ...
			   System.out.println("File type not detected for " + file.toPath());
		   }
			...
			doc.add(new LongPoint("sizeKB",fileSize));

			//creation, access and modified time.
			BasicFileAttributes fatr = Files.readAttributes(file,
					BasicFileAttributes.class);
			FileTime creationTime = fatr.creationTime();
			LocalDateTime creationT = creationTime.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime;
			doc.add(new StringField("creationTime",creationT,Field.Store.YES))

			FileTime lastAccessTime = fatr.lastAccessTime();
			LocalDateTime lastAccessT = creationTime.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime;
			doc.add(new StringField("lastAccessTime",lastAccessT,Field.Store.YES))

			FileTime lastModifiedTime = fatr.lastModifiedTime();
			LocalDateTime lastModifiedT = creationTime.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime;
			doc.add(new StringField("lastModifiedTime",lastModifiedT,Field.Store.YES))

			//Lucene filetime format
			Date creationTimeLucene = new Date(creationTime.toMillis());
			doc.add("creationTimeLucene",(DateTools.dateToString(creationTimeLucene, Resolution.SECOND)),Field.Store.YES);

			Date lastAccessTimeLucene = new Date(lastAccessTime.toMillis());
			doc.add("lastAccessTimeLucene",(DateTools.dateToString(lastAccessTimeLucene, Resolution.SECOND)),Field.Store.YES);

			Date lastModifiedTimeLucene = new Date(lastModifiedTime.toMillis());
			doc.add("lastModifiedTimeLucene",(DateTools.dateToString(lastModifiedTimeLucene, Resolution.SECOND)),Field.Store.YES);
